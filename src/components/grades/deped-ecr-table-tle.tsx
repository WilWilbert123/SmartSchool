...
import { calculateTermGrade, calculateFinalGrade3Terms, calculateItemAnalysis, transmuteGrade, getDescriptor } from "@/utils/deped-eclass-record";
...
export function DepEdECRTableTLE({ students = [] }: { students?: any[] }) {
  const [activeTab, setActiveTab] = useState<"input" | "term1" | "term2" | "term3" | "final" | "helper">("input");
  
  // Map real students into MOCK_STUDENTS format
  const mappedStudents = students.map(s => ({
    id: s.student.id,
    name: `${s.student.person.last_name}, ${s.student.person.first_name} ${s.student.person.middle_name || ""}`.trim(),
    gender: s.student.person.gender?.toUpperCase() === "FEMALE" ? "FEMALE" as const : "MALE" as const,
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-2">
          ...
          <div className="md:col-span-2 border rounded-2xl p-5 bg-card space-y-4">
            <h3 className="text-sm font-bold flex items-center justify-between border-b pb-3">
              ...
              <span className="text-xs text-muted-foreground font-normal">Registered: {mappedStudents.length} Learners</span>
            </h3>
          ...
          <table className="w-full text-xs text-center border-collapse border border-slate-900 dark:border-slate-700 border-2">
            ...
            <tbody>
              {(["MALE", "FEMALE"] as const).map((genderGroup) => {
                const studentsInGroup = mappedStudents.filter((s) => s.gender === genderGroup);
      ...
      <div className={`${activeTab === "final" ? "block" : "hidden print:block"} overflow-x-auto print:overflow-visible border rounded-2xl print:border-none print:shadow-none print:break-inside-avoid print:break-before-page print:mt-8`}>
        <div className="min-w-max print:zoom-fit">
          ...
          <div className="hidden print:flex items-center justify-between p-4 border-b bg-white text-black">
            <div className="flex items-center gap-3">
              <img src={leftLogo || "/deped-seal.png"} alt="DepEd Seal" className="h-14 w-14 object-contain" />
              <div>
            ...
            <img src={rightLogo || "/deped-logo.svg"} alt="DepEd Logo" className="h-12 object-contain" />
          </div>
